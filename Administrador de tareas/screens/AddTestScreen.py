    ...
    def add_test(self, event):
        ...
        if test_name == "":
            tk.messagebox.showinfo(
                title="ERROR",
                message="El nombre de la tarea no puede estar vacía"
            )
        else:
            try:
                ...
                tk.messagebox.showinfo(
                    title="ÉXITO",
                    message=f"La tarea {test_name} ha sido creada"
                )
            except ProgrammingError:
                tk.messagebox.showinfo(
                    title="ERROR",
                    message="Ya existe una tarea con ese nombre, crea otra"
                )
            finally:
                print(test_name)